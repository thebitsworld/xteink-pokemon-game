...
**Tiến độ**: **Toàn bộ 8 giai đoạn của roadmap chiến đấu đã xong**, cộng thêm **GĐ9-12 ngoài roadmap gốc** (commit `b4d21f31`, `eff18d13`, `f1ff1bd2`, `da111328`, `18a0eeca`, `a989d161`, `2f420118`, `5f496ad6`, `ddef31b0`, `62fd5028`, `194601a0`, `7605de43`, `060d272f`, `2fc7af7a`, `a554d92f`, `c3998aa9`, `22bb4638`, `5b9d24d2`, `19268f61`, `f2187062`, `ebb7e9fd` trên `feat/pokemon-battle-system`). **Push đang do người dùng tự làm thủ công** — máy này không có credential GitHub hoạt động (xem "Trạng thái git" bên dưới), đừng tự ý thử push nữa trừ khi được yêu cầu lại.

## ⚠️ Việc cần làm tiếp theo (GĐ13, CHƯA LÀM)

Người dùng xác nhận màn Moveset (GĐ11-12) ổn, yêu cầu ghi lại việc mới: **gym battle và bắt Pokémon hoang dã hiện chỉ cho đúng Pokémon đầu party ra đánh — hết HP là thua ngay, dù party còn Pokémon khác đủ máu.** Cần cho đổi Pokémon giữa trận; chỉ thua khi **toàn bộ party hết HP** hoặc player chủ động **RUN**.

Đã khảo sát code (chưa sửa): `setupBattlePlayer()`/`savePlayerBattleEntry()` (`PokemonActivity.cpp`) hardcode `snapshot_.party[0]`, không có khái niệm "đang dùng slot nào". `Screen::Battle` chưa có lựa chọn đổi Pokémon. Khi `OpponentWon`, `activate()` gọi thẳng `finishBattleAfterPlayerFainted()`/`finishGymChallenge(false)` kết thúc trận ngay, không kiểm tra party còn sống không. Chi tiết đầy đủ + hướng sửa gợi ý ở mục **"GĐ 13"** cuối [pokemon-battle-roadmap.md](docs/development/pokemon-battle-roadmap.md).
