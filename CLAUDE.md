...
**Tiến độ**: **Toàn bộ 8 giai đoạn của roadmap chiến đấu đã xong**, cộng thêm **GĐ9-18 ngoài roadmap gốc** (commit `b4d21f31`, `eff18d13`, `f1ff1bd2`, `da111328`, `18a0eeca`, `a989d161`, `2f420118`, `5f496ad6`, `ddef31b0`, `62fd5028`, `194601a0`, `7605de43`, `060d272f`, `2fc7af7a`, `a554d92f`, `c3998aa9`, `22bb4638`, `5b9d24d2`, `19268f61`, `f2187062`, `ebb7e9fd`, `4d4f6bbb`, `950d72ce`, `9d433ad0`, `05b623ed`, `491411e7`, `0477355e`, `470120d1`, `9b12bae6`, `188b005d`, `719fd9e2` trên `feat/pokemon-battle-system`). **Push đang do người dùng tự làm thủ công** — máy này không có credential GitHub hoạt động (xem "Trạng thái git" bên dưới), đừng tự ý thử push nữa trừ khi được yêu cầu lại.

**Điểm dừng phiên (2026-09-08)**: người dùng tự chạy giả lập, phản hồi qua nhiều vòng chỉnh màn Battle (GĐ16→GĐ17→2 bản sửa tiếp), cuối cùng xác nhận **"đẹp rồi. tạm thời dừng ở đây"**. Việc **chưa xác nhận bằng mắt** từ trước: HP bar ở `Screen::Party`/`Screen::Summary` (GĐ17).

**GĐ18 (dùng item giữa trận đấu, commit `719fd9e2`, ngoài roadmap gốc)**: thêm `Screen::BattleBag` + nút "BAG" vào menu `Screen::Battle` (FIGHT/BALL/BAG/SWITCH/RUN hoang dã, FIGHT/BAG/SWITCH/RUN gym). Tái dùng nguyên `useConsumable()`/`consumeBagItem()` có sẵn (không thêm API `PokemonService` mới), rồi đồng bộ lại `battlePlayer_` (RAM) từ `peekBattleMoves()` sau khi dùng — vì `useConsumable()` chỉ ghi xuống `BattleRecordEntry` trên đĩa qua `recordId`, không tự đụng vào combatant đang chạy trong RAM mà engine/renderer đọc. Loại Candy khỏi phạm vi (đổi level giữa trận kéo theo tính lại maxHp, phức tạp hơn). Dùng item không tốn lượt (nhất quán tiền lệ GĐ13). Flash **6,347,087 B/96.8%, còn 192,368 B**, +800 B. **Chưa xác nhận bằng mắt** — cần người dùng tự chạy giả lập dùng thử Potion giữa 1 trận thật.

**Kế hoạch đang chờ triển khai (ghi 2026-09-09)** — đọc mục **"Kế hoạch 3 giai đoạn tiếp theo (GĐ18-20)"** ở cuối roadmap trước khi bắt đầu GĐ19/20 (GĐ18 đã xong ở trên):
