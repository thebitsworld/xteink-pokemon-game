...
**Tiến độ**: **Toàn bộ 8 giai đoạn của roadmap chiến đấu đã xong**, cộng thêm **GĐ9-27 ngoài roadmap gốc** (commit `b4d21f31`, `eff18d13`, `f1ff1bd2`, `da111328`, `18a0eeca`, `a989d161`, `2f420118`, `5f496ad6`, `ddef31b0`, `62fd5028`, `194601a0`, `7605de43`, `060d272f`, `2fc7af7a`, `a554d92f`, `c3998aa9`, `22bb4638`, `5b9d24d2`, `19268f61`, `f2187062`, `ebb7e9fd`, `4d4f6bbb`, `950d72ce`, `9d433ad0`, `05b623ed`, `491411e7`, `0477355e`, `470120d1`, `9b12bae6`, `188b005d`, `719fd9e2`, `e1233cd8`, `e7c0968e`, `02fe4a15`, `6a014052`, `b808335d`, `d278b161`, `f0b49554`, `77c33495`, `fe283cb3`, `0cb2a666`, `db8e87c8`, `5ba48e7d`, `60625f11`, `9109663b`, `8b980ea0`, `566840ee`, `8c07b6b4`, `0d08f619`, `4a08c436`, `26e0b4e1`, `7b600718`, `9174a992`, `5f1d4dde`, `cbbe4ff3` trên `feat/pokemon-battle-system`). **Push đang do người dùng tự làm thủ công** — máy này không có credential GitHub hoạt động (xem "Trạng thái git" bên dưới), đừng tự ý thử push nữa trừ khi được yêu cầu lại.

**Điểm dừng phiên (2026-09-09, cập nhật)**: sau khi GĐ18 xong (xem tổng kết chi tiết bên dưới), tiếp tục triển khai hết kế hoạch 3 giai đoạn đã lập:
- **GĐ19 (`f0b49554` + follow-up `fe283cb3`)**: `Screen::BagBalls` xem số lượng bóng ngoài trận (chỉ xem, không action). Follow-up theo phản hồi: **ẩn item số lượng 0 khỏi mọi menu túi đồ** (Evolution/Medicine/Balls/TM-HM ngoài trận + BattleBag/BattleBalls giữa trận) — thêm `ownedSlotAt()`/`ownedSlotCount()` map "item thứ N đang sở hữu" về vị trí thật; `bagItemIdAt()`/`bagItemCount()` nhận thêm `bagCounts` để lọc ngay lúc duyệt. Tiện thể sửa 1 bug do chính thay đổi này gây ra: icon đá tiến hóa vẽ sai nếu không map lại index sau khi lọc.
- **GĐ19.5 (`db8e87c8`, ngoài kế hoạch, theo phản hồi)**: text thông báo dài (`Screen::Message`) và battle log tràn màn hình khi cầm dọc máy — sửa bằng `renderer.wrappedText()` (word-wrap thật) thay vì `centered()`/`truncatedText()` (không tự xuống dòng); tăng `messageHeight` 90→160 để đủ chỗ.
- **GĐ20 (`60625f11`+`9109663b`, mở rộng thành tool đa năng theo yêu cầu người dùng)**: `scripts/dev/edit_pokemon_save.py` — CLI Python chỉnh save file giả lập, subcommand `dump`/`reset-battle-store`/`reset-gym-progress`/`queue-encounter`/`set-bag-item`/`set-record-xp`. Đã tự kiểm thử toàn bộ trên save thật, xác nhận CRC hợp lệ + simulator boot sạch sau khi ghi. **Đây là công cụ sẵn dùng cho các phiên sau** — không cần viết lại kỹ thuật patch-binary/CRC32 mỗi lần cần chỉnh save nữa, chỉ cần gọi tool này. Hướng dẫn dùng đầy đủ (từng lệnh + ví dụ): [docs/development/pokemon-save-edit-tool.md](docs/development/pokemon-save-edit-tool.md).
- **GĐ21 (`8b980ea0`) → đã bị GĐ22 thay thế**: lần đầu sửa tỉ lệ rớt Ball bằng cách roll category trước rồi mới roll item (`CATEGORY_DROP_WEIGHTS`). Người dùng đánh giá **"vẫn chưa ổn"** — đúng, vì gốc rễ không phải tỉ lệ chia mà là **mọi category vẫn tranh nhau đúng 1 suất rơi mỗi giờ**. Toàn bộ cơ chế này đã bị gỡ ở GĐ22.
- **GĐ22 (`8c07b6b4`, theo yêu cầu người dùng: "4 category thu thập độc lập")**: tách hẳn thành **4 track có lịch riêng**, chỉnh cái này không ảnh hưởng cái kia.
  - **Ball**: 1 quả mỗi lần check encounter (15 phút) = **4 quả/giờ = 2.18 quả cho mỗi lần gặp Pokémon** (encounter ~1.84/giờ, mỗi con bắt tốn 2-3 quả). **Không tạo `PendingEvent`** — cấp thẳng vào túi, vì ở nhịp này bóng sẽ làm tràn hàng đợi 3 slot và bị bỏ rơi đúng lúc cần nhất. Loại bóng mở khóa dần theo tiến độ đọc: <50% chỉ Poke, ≥50% thêm Great, ≥75% thêm Ultra, ≥95% thêm Master — Master chỉ rơi khi đang có 0 quả.
  - **Medicine** (gồm Medicine+StatusCure+PPRestore+Candy, khớp đúng màn Bag): 1/2 mỗi giờ = 1 món/2 giờ.
  - **TM/HM**: 1/3 mỗi giờ = 1 cái/3 giờ.
  - **Đá tiến hóa**: giữ nguyên y hệt (1/20 + pity 19 + ưu tiên đá đang cần) = 1 viên/~12.4 giờ. Chỉ khác: fallback khi không ai cần đá giờ chỉ mở rộng trong nhóm Stone, không tràn ra 83 item — chính chỗ tràn đó là thứ làm chết nguồn cung bóng.
  - Không đổi save format/`PendingEvent`/CSV. Cập nhật 10 test cũ + 3 test mới. 19/19 pass. Flash **6,352,731 B/96.9%, còn 200,869 B**. Mọi con số cân bằng nằm gọn đầu `PokemonGame.cpp` nên chỉnh lại rất rẻ.
- **GĐ23 (`0d08f619` doc + `4a08c436` code, theo yêu cầu người dùng sau khi bàn số liệu cụ thể)**: người dùng đánh giá GĐ22 "vẫn chưa ổn" (ball 4 quả/giờ cấp cứng, không có tỉ lệ rơi thật). Sau nhiều vòng mô phỏng Monte Carlo cùng bàn bạc, chốt: **Ball 3/5 + bảo hiểm 3 lần trượt** (~2.47 quả/giờ) — cùng công thức "trượt N lần thì chắc trúng" y hệt encounter, dùng chung hàm mới `rollPityGate()` (refactor luôn encounter/đá tiến hóa vào hàm này, giảm trùng lặp). **Medicine 2/5 + bảo hiểm 3 lần trượt** ở nhịp 15 phút (không còn hourly) — StatusCure/PPRestore giảm trọng số nội bộ còn 1/3 (hàm `medicineItemWeight()`, chỉ áp dụng lúc roll, không đụng CSV) để Medicine/HP chiếm ~70.5% track thay vì ~46%. **TM/HM cùng công thức 2/5+pity3** (đủ 55 loại sau ~207h thay vì ~1183h). **Đá tiến hóa giữ nguyên y hệt** theo đúng yêu cầu ban đầu. **Save format v3(195B)→v4(198B)**: thêm 3 field `u8` (`ballMisses`/`medicineMisses`/`machineMisses`), migration tự động zero-fill, cập nhật `docs/file-formats.md` + `edit_pokemon_save.py`. Viết lại phần lớn `PokemonGameTest.cpp` (nhiều test giờ có thể khiến 4 track cùng đạt bảo hiểm và cạnh tranh 3 slot hàng đợi — thêm helper `findEventOfKind()` để test không giả định cứng vị trí slot). 19/19 test pass. Flash **6,353,021 B/96.9%, còn 200,579 B**, +290 B.

- **GĐ24 (`26e0b4e1`, theo phản hồi người dùng chơi thử)**: màn `Screen::BattleMoves` (chọn chiêu khi FIGHT) với Pokémon đủ 4 chiêu bị đè lên hộp log trận đấu — list 1 cột cũ (4 dòng × 64px) không khớp không gian HUD chừa lại. Chuyển sang lưới 2 cột giống menu Battle (`renderBattleMoveMenu()` mới, bypass `fui::list()`, tái dùng `battleMenuTop()` vì `logicalCount()` đã trả đúng số chiêu cho màn này) — hạ còn 2 dòng, hết đè. Mỗi nút: tên chiêu đậm dòng trên (`truncatedText()` phòng tên dài), PP "cur/max" dòng dưới không đậm căn phải — máy chỉ có 2 font (`UI_10`/`UI_12`), không có font nhỏ hơn nên "PP nhỏ hơn" đạt bằng độ đậm+vị trí phụ chứ không phải cỡ chữ. 19/19 test pass. Flash **6,353,495 B/96.9%, còn 200,105 B**, +474 B.

- **GĐ24 (`26e0b4e1` + sửa lần 2 `7b600718`, theo phản hồi người dùng)**: màn `Screen::BattleMoves` (chọn chiêu FIGHT) với Pokémon đủ 4 chiêu bị đè lên hộp log — chuyển sang lưới 2 cột giống menu Battle (`renderBattleMoveMenu()` mới, tái dùng `battleMenuTop()`). Sửa lần 2 theo phản hồi tiếp "PP gần ra khỏi ô": đổi từ 2 dòng (tên/PP) sang **1 hàng, căn giữa dọc**, tên trái + PP phải cùng hàng. Flash **6,353,533 B/96.9%**.
- **GĐ25 (`9174a992`, theo yêu cầu người dùng)**: `createStarter()` tặng 10 Poke Ball + 1 Potion (set thẳng `bagCounts`). HP/PP đầy đã đúng sẵn từ trước (`peekBattleMoves()` tự tổng hợp khi chưa có battle entry).
- **GĐ26 (`5f1d4dde`, theo phản hồi người dùng)**: Pokémon chỉ 1-2 chiêu làm cả HUD trận đấu (không riêng màn chọn chiêu) giãn/dịch khác hẳn so với đủ 4 chiêu, vì `battleMenuTop()` neo theo `battlePlayerMoveCount()` **thật**. Sửa: ở `Screen::BattleMoves`, `battleMenuTop()` luôn tính cố định `BATTLE_MOVE_SLOTS` (2 dòng) bất kể biết bao nhiêu chiêu — HUD giữ nguyên kích thước, chỉ số nút hiển thị đổi. Flash **6,353,555 B/96.9%, còn 200,045 B** (đo gộp cùng GĐ25).

- **GĐ27 (`cbbe4ff3`, theo phản hồi người dùng)**: layout list Party "hơi xấu" — dòng tên/level/gender do `fui::list` tự vẽ (neo theo sidePadding riêng, x≈+104) và dòng HP bar do code tự vẽ (neo theo icon, x≈+5) lệch lề nhau. Sửa: bỏ text của `fui::list` cho Party/ItemTarget-khi-hiện-HP, tự vẽ tay cả 2 dòng trong `renderPartyRowHealth()` (thêm cờ `drawNameLine`) — dòng 1 tên đậm + "Lv.N M/F" căn phải, dòng 2 HP bar+HP/HP+status, cả 2 cùng 1 lề X sau icon, căn giữa dọc trong khối. Giới tính rút gọn "M"/"F" (không có glyph ♂/♀ trong font hiện tại). `BattleSwitch` (dùng chung hàm vẽ bar, case khác) giữ nguyên qua `drawNameLine=false`. Icon loài đổi sang căn giữa dọc bằng `pokemonCenteredOffset()` thay vì offset+2 cố định. Flash **6,353,991 B/97.0%, còn 199,609 B**, +436 B.

Working tree sạch, mọi thứ đã commit, sẵn sàng để người dùng tự push. **Đã xác nhận bằng mắt GĐ19.5** (text wrap) — người dùng xác nhận "text wrap ok". GĐ20 (tool) đã hướng dẫn cách dùng. GĐ21/GĐ22 đều bị thay thế bởi vòng bàn bạc tiếp theo (không cần xác nhận riêng). **Chưa xác nhận bằng mắt**: GĐ23 (tỉ lệ rơi — hằng số nằm gọn đầu `PokemonGame.cpp` nếu cần chỉnh), GĐ24 (layout PP/tên chiêu sau sửa lần 2), GĐ25 (quà starter), GĐ26 (HUD không dịch khi ít chiêu), GĐ27 (layout Party 2 dòng cùng lề) — cần chơi thử lại để xác nhận cả 5.

**GĐ18 (dùng item giữa trận đấu — commit gốc `719fd9e2` + 6 vòng sửa/mở rộng theo phản hồi thực tế, ngoài roadmap gốc)**: tổng kết toàn bộ hành trình 1 GĐ này vì đã đổi hình dạng nhiều lần qua phản hồi liên tiếp:
1. `719fd9e2`: thêm `Screen::BattleBag` + nút "BAG" vào menu Battle, tái dùng `useConsumable()`/`consumeBagItem()` có sẵn, đồng bộ `battlePlayer_` (RAM) từ `peekBattleMoves()` sau khi dùng (vì `useConsumable()` chỉ ghi xuống đĩa qua `recordId`, không tự đụng combatant đang chạy trong RAM).
2. `e1233cd8`: sửa danh sách BattleBag (tối đa 17 item) tràn đè lên HUD — bỏ khỏi nhóm "neo đáy, đè lên HUD" (nhóm đó chỉ an toàn với ≤4 dòng), dùng list toàn màn hình canh trên thay thế.
3. `e7c0968e`: chọn item xong phải hiện danh sách party để **chọn mục tiêu** (không tự áp thẳng lên Pokémon đang đánh như bản đầu) — thêm `BagCategory::BattleMedicine`, tái dùng `Screen::ItemTarget` có sẵn.
4. `02fe4a15`: màn `ItemTarget` khi chọn mục tiêu dùng item cần hiện HP bar/text/status — `itemTargetShowsHealth()`/`showsPartyHealthRows()`.
5. `6a014052`: **đổi Pokémon hoặc dùng item giữa trận giờ tốn 1 lượt thật** (đảo ngược giản lược "miễn phí" từ GĐ13) — engine thêm `stepOpponentOnlyTurn()` (không so Speed vì switch/item luôn giải quyết trước theo đúng luật Gen 1), `PokemonService::resolveOpponentOnlyTurn()` wrapper. `Screen::BattleSwitch` cũng được thêm HP bar/text/status cùng lúc.
6. `b808335d`: menu Battle đổi thành **lưới 2 cột** (`renderBattleMenu()`, vẽ tay, không qua `fui::list()` nữa) — vừa tiết kiệm diện tích theo yêu cầu, vừa sửa tràn màn hình do BAG làm menu hoang dã lên tới 5 dòng.
7. `d278b161`: sửa nốt 1 bug đi kèm bản 2 cột — màn FIGHT (`BattleMoves`) bị đè lên HUD do `hudBottom` dùng nhầm hàm tính theo ngữ cảnh `Screen::Battle` cho cả `BattleMoves`.

Flash cuối cùng sau tất cả: **6,350,777 B/96.9%, còn 188,672 B**. **Đã xác nhận bằng mắt** toàn bộ — người dùng nói "đẹp rồi" sau vòng sửa 7.

**Kế hoạch đang chờ triển khai** (ghi 2026-09-09, chưa động tới) — đọc mục **"Kế hoạch 3 giai đoạn tiếp theo (GĐ18-20)"** ở cuối roadmap trước khi bắt đầu:
...
- **GĐ20**: script Python chỉnh trực tiếp `fs_/.crosspoint/pokemon-{a,b}.bin` (giả lập) để chèn sẵn `PendingEventKind::Encounter` (offset/luật hợp lệ đã ghi chi tiết trong roadmap), giúp test màn bắt Pokémon ngay không cần chờ đọc sách đủ lâu. **Đã thực hiện thủ công 1 lần trong phiên 2026-09-09** (không qua script chính thức — chèn tay 3 encounter Pidgey/Rattata/Caterpie vào `fs_/.crosspoint/pokemon-{a,b}.bin` để test màn bắt) nhưng chưa viết thành script tái dùng được ở `scripts/dev/` như kế hoạch — nếu cần lại, hoặc làm thủ công tương tự hoặc viết script chính thức lúc đó.
