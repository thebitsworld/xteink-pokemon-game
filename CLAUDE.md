...
- **This machine (Windows) now has a working build toolchain** — see the dedicated "Windows build environment" section below for the exact setup and the gotchas that cost real time to work through (Python version, wrong `pio` binary, console encoding, a destructive `pio pkg install` flag, and — the real root cause behind several of the earlier failures — leaked `MSYSTEM`/MinGW environment variables). A full `pio run -e pokemon-x3` build has been verified working end to end on this machine as of this session.
...
## Windows build environment (this machine, set up 2026-09-12)
