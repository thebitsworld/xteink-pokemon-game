...
- [ ] **Chưa xác nhận bằng mắt** — cần người dùng tự đọc sách đủ lâu trên giả lập (hoặc dùng `queue-encounter`/`set-bag-item` của tool GĐ20 để giả lập nhanh) để xác nhận cảm giác rớt bóng đã dễ chịu hơn; nếu vẫn thấy chậm, chỉnh thêm số trong `CATEGORY_DROP_WEIGHTS` (`lib/Pokemon/PokemonGame.cpp`) là đủ, không cần đổi CSV.
