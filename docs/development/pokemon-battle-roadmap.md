...
- [ ] **Chưa xác nhận bằng mắt** — cần chơi thử để xác nhận nhịp rơi thực tế; mọi con số đều nằm gọn ở đầu `PokemonGame.cpp` (`MEDICINE_DROP_DENOMINATOR`, `MACHINE_DROP_DENOMINATOR`, ngưỡng progress trong `ballKindDroppable()`, và nhịp ball = `ENCOUNTER_CHECK_MINUTES`) nên chỉnh lại rất rẻ, không cần đụng CSV.
