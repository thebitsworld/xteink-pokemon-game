...
### GĐ 18 — Dùng item (Bag) giữa trận đấu ⏳ CHƯA LÀM

**Vấn đề**: `Screen::Battle` hiện chỉ có FIGHT/BALL/SWITCH/RUN (hoang dã) hoặc FIGHT/SWITCH/RUN (gym) — **không có cách nào dùng Potion/thuốc giải status giữa trận**. `PokemonService::useConsumable()` (đã có từ GĐ9) chỉ được gọi từ `Screen::BagMedicine` (`PokemonActivity.cpp:913`), một màn hình ngoài trận, không có đường vào từ `Screen::Battle`.

**Phát hiện quan trọng cần xử lý**: `useConsumable()` (`PokemonService.cpp:384-443`) với category Medicine/StatusCure/PPRestore đọc/ghi thẳng `BattleRecordEntry` **trên đĩa** qua `battleStore_.upsertEntry()` (dùng `recordId`, không đụng gì tới `battlePlayer_` đang nằm trong RAM của `PokemonActivity`). Nếu gọi thẳng hàm này giữa trận mà không đồng bộ lại, `battlePlayer_.currentHp/status/statusTurns/moves[].currentPp` (dùng bởi engine `stepBattle()` và bởi `renderBattleHud()`) sẽ **không cập nhật** — HP đã hồi trên file nhưng màn hình vẫn hiện HP cũ. Category `Candy` thì mutate `PokemonRecord.totalXp` ở **save chính** (không phải battle store) — tăng cấp giữa trận kéo theo phải tính lại `maxHp` sống của `battlePlayer_`, phức tạp hơn hẳn — **cố tình loại `Candy` khỏi phạm vi giai đoạn này**, chỉ cho dùng Medicine/StatusCure/PPRestore giữa trận.

- [ ] Thêm dòng "BAG" vào menu `Screen::Battle` — cả hoang dã (chèn giữa BALL và SWITCH: FIGHT/BALL/BAG/SWITCH/RUN, 5 dòng) lẫn gym (FIGHT/BAG/SWITCH/RUN, 4 dòng — BAG dùng được ở cả 2 loại trận, khác BALL chỉ hoang dã). Sửa `logicalCount()`, label switch (`:1493-1521` khu vực cũ), và selection handler (`:1016-1043` khu vực cũ) — chú ý index dịch chuyển khi thêm 1 dòng mới, đặc biệt logic `isGym` hiện tại tính index BALL/SWITCH tương đối, phải rà lại toàn bộ thay vì chỉ chèn giữa.
- [ ] `Screen::BattleBag` (màn mới) — liệt kê item dùng được giữa trận: filter theo predicate mới (Medicine/StatusCure/PPRestore, **loại Candy**) — viết hàm predicate riêng (không tái dùng `isMedicineCategory` hiện có vì nó gồm cả Candy), tái dùng `bagItemIdAt()`/`bagItemCount()` (đã tổng quát hóa qua con trỏ hàm predicate từ GĐ9) với predicate mới này.
- [ ] Chọn 1 item trong `Screen::BattleBag`: gọi `service_.useConsumable(snapshot_.party[battlePartySlot_].recordId, itemId)` như bình thường, rồi **đồng bộ lại `battlePlayer_`** bằng cách gọi lại `service_.peekBattleMoves(record)` ngay sau đó và copy `currentHp`/`status`/`statusTurns`/`moves[].currentPp` vào `battlePlayer_` (đọc thuần, không ghi thêm — entry vừa ghi ở bước trên rồi). Không có API mới ở `PokemonService` — chỉ tái dùng 2 hàm đã có.
- [ ] **Giản lược có chủ đích, nhất quán với tiền lệ GĐ13** (đổi Pokémon không tốn lượt): dùng item giữa trận cũng **không tốn lượt** — không cho đối thủ đánh miễn phí. Engine `stepBattle()` không có khái niệm "item action", thêm cơ chế lượt riêng chỉ để mô phỏng đúng luật Red sẽ phải sửa engine đã ổn định/có test — đánh đổi lấy đơn giản, đúng tinh thần các giản lược trước.
- [ ] 1 key i18n mới (nếu cần) cho tiêu đề màn `Screen::BattleBag` — có thể tái dùng thẳng `STR_POKEMON_BAG` ("Bag") đã có sẵn, không cần thêm key.
- [ ] Chỉ sửa `PokemonActivity.cpp/.h` — không đụng `PokemonService`/storage/test native (đúng pattern các GĐ UI-only trước, GĐ13/16/17).
- [ ] Build simulator + smoke test + `pio run -e pokemon-x3` đo flash, theo đúng quy trình mọi GĐ trước.
