...
### GĐ 27 — Căn chỉnh lại layout list Party: icon trái, 2 dòng phải cùng lề (theo phản hồi người dùng) ✅ XONG (commit `cbbe4ff3`)

**Yêu cầu**: "layout hiển thị list pokemon trong party đang hơi xấu. căn chỉnh lại cho đẹp hơn. bên trái là ảnh pokemon, bên phải là 2 dòng text gồm dòng 1 là tên - lvl - giới tính dạng symbol, dòng 2 là HP bar - HP/HP - Status". Nguyên nhân gốc của "xấu": dòng tên/level/gender do widget `fui::list` tự vẽ (neo theo `sidePadding` riêng của nó, x≈+104) trong khi dòng HP bar do `PokemonActivity` tự vẽ tay (neo theo icon, x≈+5) — hai dòng lệch lề trái với nhau thay vì đọc như 1 khối gọn gàng.

- [x] Bỏ qua hoàn toàn text của `fui::list` cho `Screen::Party` và `Screen::ItemTarget` (khi đang hiện HP, `showsPartyHealthRows()`), tự vẽ tay **cả 2 dòng** trong `renderPartyRowHealth()` (đổi chữ ký, thêm cờ `drawNameLine`): dòng 1 tên (đậm) + "Lv.N M/F" căn phải, dòng 2 HP bar + HP/HP + status — cả 2 dòng cùng bắt đầu 1 tọa độ X ngay sau icon, căn giữa theo chiều dọc trong khối 2 dòng (giống cách làm HP bar ở Battle/Summary, GĐ17).
- [x] Giới tính rút gọn **"M"/"F"** (`genderAbbrev()` mới) thay vì chữ đầy đủ ("Male"/"Female") — **máy không có glyph ♂/♀ trong font hiện tại** (font subset Latin/Hebrew/Arabic cố định dựng lúc build, không có 2 ký tự này), nên chữ cái đơn là "symbol" gọn nhất có sẵn mà không phải build lại font (tốn thêm bitmap glyph + risk flash).
- [x] `Screen::BattleSwitch` (dùng chung `renderPartyRowHealth()` để vẽ HP bar nhưng là case `buildRows()` khác, có value string riêng) giữ nguyên hành vi cũ hệt — chỉ truyền `drawNameLine=false` để không vẽ đè tên 2 lần.
- [x] Icon loài Pokémon (dùng chung nhiều màn hình: Party/Move/ItemTarget/Pc/Pokedex/BattleSwitch) đổi từ offset `+2` cố định sang căn giữa theo chiều dọc bằng `pokemonCenteredOffset()` — dòng 64px không đổi kết quả (2 = (64-60)/2 vốn đã đúng), chỉ cải thiện các dòng cao 96px (Party/ItemTarget-HP/BattleSwitch).
- [x] Chỉ sửa `PokemonActivity.cpp/.h`. 19/19 test native pass (không đổi logic ngoài UI). Flash **6,353,991 B (97.0%, còn 199,609 B)** — +436 B.
- [x] **Đã xác nhận bằng mắt** (qua 4 vòng sửa layout HP bar/marker) — người dùng xác nhận "ok rồi" sau bản sửa lần 4.

### GĐ 28 — Màn Settings, đổi "Reset Pokémon" thành "Reset Game", xóa kèm battle store (theo yêu cầu người dùng) ✅ XONG (commit `94dfa6c7`)

**Bối cảnh**: người dùng hỏi "nút reset pokemon sẽ làm những gì" — khi tra code để trả lời, phát hiện `PokemonService::reset()` chỉ xóa save chính (`pokemon-{a,b}.bin`), **không đụng** file phụ `pokemon-battle-{a,b}.bin` (HP/PP/moveset đang sống theo `recordId`). Vì `createStarter()` luôn gán cứng `recordId=1` và mỗi Pokémon bắt sau lấy `id=recordCount()+1`, sau khi reset các Pokémon mới sẽ lại mang đúng id cũ (1,2,3...) từ lượt chơi trước — nếu id đó từng đánh trận, Pokémon mới sẽ đọc nhầm HP/PP/moveset cũ cho tới khi tự đánh trận thật ghi đè lại. Người dùng yêu cầu: gộp Reset vào 1 mục Settings mới, đổi tên thành "Reset Game", và sửa luôn bug xóa-thiếu-file này.

- [x] **Màn `Screen::Settings` mới**: Menu gốc rút còn 8 mục (bỏ "Show on Home Screen" + "Reset Pokémon", gộp cả 2 vào Settings). Settings có 2 dòng: Home Screen toggle (dời nguyên logic từ Menu) + "Reset Game" (đổi tên từ "Reset Pokémon", tái dùng đúng key `STR_POKEMON_RESET` — chỉ 1 nơi dùng key này nên đổi text an toàn). `ResetFirst`/`ResetFinal` giờ quay lại Settings khi hủy/lỗi thay vì nhảy thẳng về Menu (`goBack()` cũng thêm case riêng). Thêm `STR_POKEMON_SETTINGS` ("Settings") mới.
- [x] **Sửa bug xóa thiếu battle store**: thêm `PokemonBattleStore::reset()` (mới) — ghi `BattleStoreState{}` rỗng qua `writeState()` có sẵn, đúng quy ước double-buffer hiện có (ghi file không-active trước, xác minh đọc lại, rồi mới chuyển active — không cần logic mới, chỉ gọi lại hạ tầng đã có). `PokemonService::reset()` gọi thêm bước này sau khi xóa save chính — **best-effort**: nếu riêng bước này thất bại vẫn báo `reset()` thành công, vì save chính (phần người chơi thấy trực tiếp) đã xóa xong là phần quan trọng nhất, và 1 entry battle-store sót lại sẽ tự bị ghi đè ngay lần đầu Pokémon đó đánh trận thật.
- [x] Thêm test `ResetAlsoClearsTheBattleStoreSoTheNextStarterDoesNotInheritLeftoverBattleData` (`PokemonServiceTest.cpp`) — tạo starter cũ (Pikachu), gây thương tích+status, reset, tạo starter mới (Bulbasaur, cố tình khác loài để dễ phát hiện nếu lẫn dữ liệu, cùng `recordId=1`), xác nhận HP/PP/status đều mới tinh chứ không kế thừa dữ liệu cũ.
- [x] Chỉ sửa `PokemonActivity.cpp/.h`, `PokemonService.cpp`, `PokemonBattleStore.h/.cpp`, `english.yaml` + test liên quan. 19/19 test native pass (thêm 1 test mới). Flash **6,354,251 B (97.0%, còn 199,349 B)** — +216 B.
- [ ] **Chưa xác nhận bằng mắt** — cần chơi thử: vào Menu > Settings, xác nhận thấy đúng 2 mục (Home Screen + Reset Game), thử Reset Game và xác nhận quay lại màn chọn starter đúng như trước.
