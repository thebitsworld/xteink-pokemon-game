...
- Boxed Pokémon no longer lose their TM/HM moves and PP Ups. Every Pokémon's moveset and PP Ups are now kept in their own file (`pokemon-moves-a/b.bin`, up to the full PC Box), instead of sharing the 6 Party-sized slots of the battle file - which forced a boxed Pokémon's data out whenever you swapped enough Pokémon in and out. What can still be reset in that rare case is only its current HP/PP/status. Existing saves are upgraded automatically the first time it matters; nothing changes in the main save, and older firmware simply ignores the new file.
- A momentary SD-card write hiccup while a party member's battle data was being created could wipe a different, boxed Pokémon's saved data. Old data is now only replaced when the store is genuinely full.
